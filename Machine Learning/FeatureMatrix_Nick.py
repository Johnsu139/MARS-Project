...
dataFiles_labels = 'Nick_Epoch.csv'
nSubjects_Nick = 7; # Number of time segments in AWW resting dataset
...
def featureMatrix(data,labels_all): # Construct the feature matrix
    ...
    ACCx = data[0:length*40,2].reshape(length,40)
    ACCy = data[0:length*40,3].reshape(length,40)
    ACCz = data[0:length*40,4].reshape(length,40)
...
NickLabels = dict()
NickGroups = dict()
data_Nick = np.loadtxt(dataFiles, delimiter=',', skiprows=2)
labels_Nick = np.loadtxt(dataFiles_labels, delimiter=',', skiprows=1, usecols=(1))
labels_Nick[labels_Nick==0]=1 # Assume the unlabeled time windows as clean
labels_Nick = labels_Nick-1 # Make the labels include only 0s and 1s
NickGroups = np.ones(len(labels_Nick)) # The group number for the leave one group out cross-validation
NickAll,NickAcc,NickEda,NickLabels = featureMatrix(data_Nick,labels_Nick)


np.savetxt('NickAll.csv', NickAll, delimiter=',')
np.savetxt('NickAcc.csv', NickAcc, delimiter=',')
np.savetxt('NickEda.csv', NickEda, delimiter=',')
np.savetxt('NickLabels.csv', NickLabels, delimiter=',')